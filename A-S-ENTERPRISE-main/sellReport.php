                        ...
                        <li>
                            ...
                            <div class="collapse" id="home-collapse">
                                <ul class="btn-toggle-nav list-unstyled fw-normal ms-5 p-1 small">
                                    <li>
                                        <a href="./sellReport.php" class="active fs-7 nav-link d-flex align-items-center p-2 text-light">
                    ...
                    <!-- google graph bar -->
                     <div class="container row">
                        ...
                        <div class="col d-flex justify-content-center align-items-center flex-row">
                            ...
                            <a href="generate_pdf.php?table=sell"class="btn btn-primary bg-primary text-light rounded-5">VIEW PDF</a>
                                ...
                                <?php
                                        ...
                                        if ($result1->num_rows > 0) {
                                            ...
                                            while($row = $result1->fetch_assoc()) {
                                                ...
                                                if (empty($row['clearAmount']) || is_null($row['clearAmount'])) {
                                                    // If clearAmount is NULL or empty, set status to UNPAID
                                                    $status = 'UNPAID';
                                                } elseif ($row['totalAmount'] == $row['clearAmount']) {
                                                    // If totalAmount equals clearAmount, set status to PAID
                                                    $status = 'PAID';
                                                } elseif ($row['totalAmount'] > $row['clearAmount']) {
                                                    // If totalAmount is greater than clearAmount, set status to DUE
                                                    $status = 'DUE';
                                                } else {
                                                    // For any other case, set status to UNPAID
                                                    $status = 'UNPAID';
                                                }
                                                

                                                // Update the status in the database
                                                $sql_update_status = "UPDATE sell SET status = '$status' WHERE id=$id";
                                                $conn->query($sql_update_status); // Execute the update query
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#rangeShow').click(function(){
                ...
                $('#sellDetails tbody tr').each(function(){
